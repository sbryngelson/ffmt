...
pub fn align_declarations(lines: &[String]) -> Vec<String> {
    ...
    while i < len {
        ...
        if group_size >= 2 {
            ...
            // We want to align based on the position within the trimmed content
            // (after indent), then re-add the indent.
...
mod tests {
    ...
    fn test_basic_alignment() {
        ...
        let result = align_declarations(&input);
